...
namespace DInterests
{
    ...
    public class InterestDef : Def
    {
        ...
        private readonly float
            value = float
                .MinValue; // used to determine which passion is the "best" for purposes of the colonist preparation tab
        ...
        public float
            inspirationFactor =
                1.0f; // determines how likely the associated skill is to be chosen for inspiration if an inspiration occurs
        ...
        public float
            weight = 8.8f; // default formula = weight + weightLevelFactor * weightLevelExponent^level + weightLevelOffset*level;

        public float weightLevelExponent = 1.0f;
        public float weightLevelFactor = 1.0f;
        public float weightLevelOffset = 0.0f;

        public static bool operator <(InterestDef a, InterestDef b)
        {
        ...
        public virtual void drawWorkBox(Rect rect)
        {
            ...
            var tex = ContentFinder<Texture2D>.Get(drawTex);
        ...
        public virtual float GetWeight(SkillRecord skill, Pawn pawn)
        {
            ...
            var levelTerm = weightLevelFactor * (float)Math.Pow(weightLevelExponent, level);
        ...
        public virtual void RecordMasterTale(SkillRecord sr, Pawn p)
        {
            TaleRecorder.RecordTale(
                GetValue() <= 0 ? TaleDefOf.GainedMasterSkillWithoutPassion : TaleDefOf.GainedMasterSkillWithPassion, p,
                sr.def);
        }
        ...
        public virtual int InterestCountForPawn(Pawn pawn)
        {
            ...
            var interest = InterestBase.interestList[defName];
            var count = 0;
            foreach (var sr in pawn.skills.skills)
            {
                if ((int)sr.passion == interest)
                {
                    ++count;
                }
            }
        ...
        {
            var curLevel = ih.pawn.needs.mood.CurLevel;
